...
class PersonaDao implements IPersonaDao {
    ...
    public function insert($persona) {
        ...
        try {
            $sql = "INSERT INTO `persona`( `id`, `nombre`, `telefono`, `correo`, `perfiles_id`)"
                    . "VALUES ('$id','$nombre','$telefono','$correo','$perfiles_id')";
    ...
    public function select2($persona) {
    ...
    public function listAll2($correo) {
        $lista = array();
        try {
            $sql = "SELECT `id`, `nombre`, `telefono`, `correo`, `perfiles_id`"
                    . "FROM `persona`"
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                $persona->setTelefono($data[$i]['telefono']);
                $persona->setCorreo($data[$i]['correo']);
                $persona->setPerfiles_id($data[$i]['perfiles_id']);
