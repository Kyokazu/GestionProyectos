...
function iniciarTabla() {
    $("#listadoTablaFO_IN_12_DD").DataTable({
        responsive: true,
        ordering: true,
        paging: true,
        searching: true,
...
function obtenerDatos() {
    ...
    fetch("../../back/controller/Cumplimiento_acompanamientoController_List.php", {
        method: "GET",
        headers: {
...
function listadoSolicitudes(solicitudes) {
    let tabla = $("#listadoTablaFO_IN_12_DD").DataTable();
    tabla.data().clear();
...
function construirSelectProyectos(pro) {
    ...
    for (let index = 0; index < pro.length; index++) {
        let gpI = pro[index],
                opcion = new Option(gpI.nombre_tutor, gpI.id);
        if (gpI.estado_solicitud === "En trámite") {
            $(opcion).html(gpI.nombre_tutor);
            input.append(opcion);
        }
    }
}
...
function aprobarSolicitudHorasTutor_DD() {
    ...
    Utilitario.agregarMascara();
    fetch("../../back/controller/Cumplimiento_acompanamientoController_Update.php", {
        method: "POST",
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
