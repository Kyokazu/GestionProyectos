...
function iniciarTabla() {
    $("#listadoTablaFO_IN_09").DataTable({
        responsive: true,
        ordering: true,
        paging: true,
        searching: true,
...
function listadoSolicitudes(solicitudes) {
    let tabla = $("#listadoTablaFO_IN_09").DataTable();
    tabla.data().clear();
...
function obtenerDatosCombo() {
    let ordenes = {
        id_solicitud: $('#combo_solicitudesF').val()
    };
    Utilitario.agregarMascara();
...
function construirSelectProyectos(pro) {
    $("#combo_solicitudesF").empty();
    let input = $("#combo_solicitudesF");
    let opcion = new Option("Seleccione la solicitud", "-1");
    $(opcion).html("Seleccionea solicitud");
    input.append(opcion);
...
function aprobarSolicitudInformeGestion_DD() {
    ...
    Utilitario.agregarMascara();
    fetch("../../back/controller/Concepto_cumplimientoController_Update.php", {
        method: "POST",
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
